...
const FilterBar = memo((props: Props) => {
  const { onFilterToQueryString, filterParams, provinceId } = props;
  const dispatch = useDispatch();
  const { actions } = useClaimedClubListSlice();
  const { exporting, provinces } = useSelector(selectClaimedClubList);
  ...
  useEffect(() => {
    ...
    if (filterParams?.searchField) {
      setSearchValue(filterParams.searchField);
    }
  ...
  const exportClubClaimList = () => {
    dispatch(
      actions.exportClubClaimList(
        {
          provinceId:
