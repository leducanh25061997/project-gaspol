...
export const ClubManagements = ({
  ...
  useEffect(() => {
    if (clubMangements.every((role: any) => role.userUuid && role.phone)) {
      setChecked(true);
    }
    if (clubMangements[0] && clubMangements[0].fullName) {
      setDataAdmin(clubMangements[0]);
      setIsCLubAdmin(true);
    }
    setValue('clubAdmin', clubMangements[0] && clubMangements[0].fullName);
    setValue('finance', clubMangements[1] && clubMangements[1].fullName);
    setValue('president', clubMangements[2] && clubMangements[2].fullName);
    setValue('vicePresident', clubMangements[3] && clubMangements[3].fullName);
    setValue('secretary', clubMangements[4] && clubMangements[4].fullName);
    setValue('humanResource', clubMangements[5] && clubMangements[5].fullName);
  }, [clubMangements]);
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      ...
      const newClubs = clubMangements.map((item: any) => {
    ...
    <Card sx={{ mt: 3, padding: '1rem' }}>
      ...
      <Collapse in={collapse} timeout="auto" unmountOnExit>
        ...
        <Grid container spacing={2} justifyContent="center" mt={1}>
          {clubMangements
            ?.filter((clubManagement: any) => !!clubManagement)
            ?.map((clubMangement: any, i: number) => {
              return (
