...
export const TableList = memo((props: Props) => {
  ...
  const { changeHistoryClub, isLoadingChangeHistory } =
  ...
  const renderItem = (item: ChangeHistory, index?: number) => {
    return [
      <Stack direction="row" alignItems="center" spacing={3}>
        ...
        <EllipsisText
          text={item?.ktaName ? String(item?.ktaName) : ''}
          line={2}
        ></EllipsisText>
      </Stack>,
      item.oldClubName
  ...
  return (
    <div>
      <Card sx={{ padding: '1rem', mt: 3 }}>
        <Header
          title={t(translations.report.changeHistory)}
        ...
        <Table
          headers={headers}
          items={changeHistoryClub?.data}
          pageNumber={filter.page}
          totalElements={changeHistoryClub?.count}
          ...
          onSelectRow={item => {
            if (
              ...
              navigate(path.memberships + `/${item?.ktaId}`, {
                state: { id: item.ktaId },
              });
            }
          }}
      ...
      <Loading isLoading={isLoadingChangeHistory} />
    </div>
  );
});
